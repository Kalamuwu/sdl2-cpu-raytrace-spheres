...
{
    int x = index % WINDOW_WIDTH;
    int y = index * (1.0f / WINDOW_WIDTH);
    // printf("(%d,%d) starting.\n", x, y);
    ...
    uint8_t ir = uint8_t(sqrt(col[0]) * 255.99f);
    uint8_t ig = uint8_t(sqrt(col[1]) * 255.99f);
    uint8_t ib = uint8_t(sqrt(col[2]) * 255.99f);
